...
fn get_vreg(machine: &Machine, name: &str) -> Vec<u8> {
    let reg = machine.lookup_register(name).unwrap();
    let hart = machine.get_default_hart().unwrap();
    match hart.read_register(&reg) {
...
fn set_vreg(machine: &mut Machine, name: &str, bytes: Vec<u8>) {
    let reg = machine.lookup_register(name).unwrap();
    let hart = machine.get_default_hart_mut().unwrap();
...
fn get_velem(vreg: &[u8], elem_idx: usize, sew: usize) -> u64 {
...
fn set_velem(vreg: &mut [u8], elem_idx: usize, sew: usize, value: u64) {
    let offset = elem_idx * sew;
    for b in 0..sew {
        if offset + b < vreg.len() {
...
fn setup_vsetvli(machine: &mut Machine, sew: &str, _lmul: &str, vl: u64) {
    let hart = machine.get_default_hart_mut().unwrap();
